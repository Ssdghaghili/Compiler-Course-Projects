...
declaration : (STATIC | SHARED)? (primitiveType | complexType) (LBRACK INT_LITERAL RBRACK)? ID {if ($INT_LITERAL.text != null) System.out.println("ArrayDec:"+$ID.text+":"+$INT_LITERAL.text); else System.out.println("VarDec:"+$ID.text);} (ASSIGN expression {System.out.println("Operator:=");})? SEMICOLON;
...
assignment : ID (LBRACK expression RBRACK)? assign expression {System.out.println("Operator:"+$assign.text);} SEMICOLON;
